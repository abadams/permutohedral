...
#  define CU_SAFE_CALL_NO_SYNC( call ) do {                                  \
    CUresult err = call;                                                     \
    if( CUDA_SUCCESS != err) {                                               \
        fprintf(stdout, "Cuda driver error %x in file '%s' in line %i.\n",   \
...
#  define CUFFT_SAFE_CALL( call) do {                                        \
    ...
    if( CUFFT_SUCCESS != err) {                                              \
        fprintf(stdout, "CUFFT error in file '%s' in line %i.\n",            \
                __FILE__, __LINE__);                                         \
	fflush(stdout);							\
        exit(EXIT_FAILURE);                                                  \
    } } while (0)
...
#  define CUT_SAFE_CALL( call)                                               \
    if( CUTTrue != call) {                                                   \
        fprintf(stdout, "Cut error in file '%s' in line %i.\n",              \
                __FILE__, __LINE__);                                         \
	fflush(stdout);							\
        exit(EXIT_FAILURE);                                                  \
    } 
...
#  define CUT_CHECK_ERROR(errorMessage) do {                                 \
    ...
    if( cudaSuccess != err) {                                                \
        fprintf(stdout, "Cuda error: %s in file '%s' in line %i : %s.\n",    \
...
#  define CUT_SAFE_MALLOC( mallocCall ) do{                                  \
    if( !(mallocCall)) {                                                     \
        fprintf(stdout, "Host malloc failure in file '%s' in line %i\n",     \
                __FILE__, __LINE__);                                         \
	fflush(stdout);							\
        exit(EXIT_FAILURE);                                                  \
    } } while(0);
...
#  define CUT_DEVICE_INIT(ARGC, ARGV) {                                      \
    ...
    if (deviceCount == 0) {                                                  \
        fprintf(stdout, "cutil error: no devices supporting CUDA.\n");       \
        exit(EXIT_FAILURE);                                                  \
    } else {								\
	fprintf(stdout, "%i devices found.\n", deviceCount);		\
    }									\
    int dev = 0;                                                             \
    ...
    if (deviceProp.major < 1) {                                              \
        fprintf(stdout, "cutil error: device does not support CUDA.\n");     \
        exit(EXIT_FAILURE);                                                  \
    }                                                                        \
